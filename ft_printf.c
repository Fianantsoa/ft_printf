...
static void	ft_print_arg(char type, va_list args, size_t *count)
{
	...
	*count += get_print_len(type, copy);
	...
	{
		ft_putstr_fd("0x", 1);
		ft_putstr_fd(ft_ltoh((unsigned long) va_arg(args, void *)), 1);
	}
	if (type == '%')
		ft_putchar_fd('%', 1);
	if (type == 'u')
	...
	if (type == 'x')
		ft_putstr_fd(ft_ltoh(va_arg(args, unsigned int)), 1);
	if (type == 'X')
		ft_putstr_fd(ft_strupcase(ft_ltoh(va_arg(args, unsigned int))), 1);
}
